module.exports = function(app){
	var ComentariosController = {
		...
		updateComentarios : function(request,response){
			...
			var sql = 'UPDATE `tb_comentarios` SET `id_usuario_app`="'+comment.id_usuario_app+'",`horario`="'+comment.horario+'",`texto_comentario`="'+comment.texto_comentario+'",`tipo_comentario`="'+comment.tipo_comentario+'",`id_referencia`="'+comment.id_referencia+'" WHERE  `id_comentario` = "'+commentId+'"';
			app.get('db').query(sql, function(err,rows){
				if (err == null){
					response.json(rows)
				} else {
		...
		deleteComentarios : function(request,response){
			var commentId = request.params.commentId;
			var comment = request.body;
			var sql = 'UPDATE `tb_comentarios` SET `dismist`= 1 WHERE `id_comentario` = "'+commentId+'"';
			app.get('db').query(sql, function(err,rows){
				if (err == null){
					response.json(rows)
				} else {
